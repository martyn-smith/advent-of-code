...
impl Instruction {
    fn new(l: &str) -> Self {
        ...
        let idxs : Vec<Match> = get_nums.find_iter(l)
                        .collect();
        Instruction {
            ...
            start: {let x: Vec<&str> = l[idxs[0].start()..idxs[0].end()].split(',').collect();
                        (x[0].parse::<usize>().unwrap(),
                         x[1].parse::<usize>().unwrap())
            },
            end: {let x: Vec<&str> = l[idxs[1].start()..idxs[1].end()].split(',').collect();
                        (x[0].parse::<usize>().unwrap(),
                         x[1].parse::<usize>().unwrap())
            }
        }
        ...
        for i in instr.start.0..=instr.end.0 {
            for j in instr.start.1..=instr.end.1 {
                match instr.cmd {
                    ...
                    Action::Off => {lights[i][j] = lights[i][j].saturating_sub(1);},
