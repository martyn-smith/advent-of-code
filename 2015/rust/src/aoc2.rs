...
fn paper(dims: &(usize, usize, usize)) -> usize {
    ...
    2 * sides.0 + 2 * sides.1 + 2 * sides.2 + min(min(sides.0, sides.1), sides.2)
}
...
fn ribbon(dims: &(usize, usize, usize)) -> usize {
    ...
    min(min(perimeters.0, perimeters.1), perimeters.2) + volume
}
...
pub fn part_1(dims: &[(usize, usize, usize)]) -> usize {
    dims.iter().map(paper).sum()
}

pub fn part_2(dims: &[(usize, usize, usize)]) -> usize {
    dims.iter().map(ribbon).sum()
}
