...
class Ferry:
    ...
    def count_occupied(self, i, j) -> int:
        ...
        if self.visibility == "distant":
            cells_to_check = []
            try: #looking up
                cells_to_check.append((next(ix for ix in range(i-1, -1, -1) if self.prev_state[ix][j] != self.floor), j))
            except StopIteration:
                pass
            try: #looking left
                cells_to_check.append((i, next(jx for jx in range(j-1, -1, -1) if self.prev_state[i][jx] != self.floor)))
            except StopIteration:
                pass
            try: #looking down
                cells_to_check.append((next(ix for ix in range(i+1, self.dimensions[0]) if self.prev_state[ix][j] != self.floor), j))
            except StopIteration:
                pass
            try: #looking right
                cells_to_check.append((i, next(jx for jx in range(j+1, self.dimensions[1]) if self.prev_state[i][jx] != self.floor))) 
            except StopIteration:
                pass
            try: #looking up-left
                cells_to_check.append(next((ix, jx) for ix, jx in zip(range(i-1, -1, -1), range(j-1, -1, -1)) if self.prev_state[ix][jx] != self.floor))
            except StopIteration:
                pass
            try: #looking up-right
                cells_to_check.append(next((ix, jx) for ix, jx in zip(range(i-1, -1, -1), range(j+1, self.dimensions[1])) if self.prev_state[ix][jx] != self.floor))
            except StopIteration:
                pass
            try: #looking down-left
                cells_to_check.append(next((ix, jx) for ix, jx in zip(range(i+1, self.dimensions[0]), range(j-1, -1, -1)) if self.prev_state[ix][jx] != self.floor))
            except StopIteration:
                pass
