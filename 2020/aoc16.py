...
def invalid_fields(ticket) -> list:
    return [
        ...
        for t in ticket
        if not any(
            t in range(f[1], f[2] + 1) or t in range(f[3], f[4] + 1) for f in fields
        )
    ]
...
def valid_fields(ticket) -> list:
    return [
        [
            f[0]
            for f in fields
            if t in range(f[1], f[2] + 1) or t in range(f[3], f[4] + 1)
        ]
        for t in ticket
    ]
...
def get_indices() -> list:
    valid_ticket_fields = [
        valid_fields(t) for t in nearby_tickets if invalid_fields(t) == []
    ]
