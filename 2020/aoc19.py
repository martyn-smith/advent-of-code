...
def builder(rules, root="0") -> str:
    ...
    if b := base_rule.match(rule):
        ...
        return "(" + builder(rules, lc.group(2)) + "|" + builder(rules, lc.group(3)) + ")"
    elif c := compound_rule.match(rule):
        return "(" + builder(rules, c.group(2)) + builder(rules, c.group(3)) + "|" + builder(rules, c.group(4)) + builder(rules, c.group(5)) + ")"
