                ...
                for x in range(len(grid[0]) - len(monster[0])):
                    candidate_monster = [
                        "".join(
                            (
                                "#"
                                if grid[y + i][x + j] == "#" and monster[i][j] == "#"
                                else " "
                            )
...
class Tile:
    ...
    def matching_edges(self, tiles):
        ...
        top_match = any(
            self.top_edge in t.all_edges or self.top_edge[::-1] in t.all_edges
            for t in tiles
        )
        right_match = any(
            self.right_edge in t.all_edges or self.right_edge[::-1] in t.all_edges
            for t in tiles
        )
        bottom_match = any(
            self.bottom_edge in t.all_edges or self.bottom_edge[::-1] in t.all_edges
            for t in tiles
        )
        left_match = any(
            self.left_edge in t.all_edges or self.left_edge[::-1] in t.all_edges
            for t in tiles
        )
