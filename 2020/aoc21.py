...
for line in lines:
    ...
    for allergen in line_allergens:
        if allergen not in all_allergens:
            ...
            all_allergens[allergen] = all_allergens[allergen].intersection(set(line_ingredients))
    ...
    for allergen in all_allergens:
        if len(all_allergens[allergen]) == 1:
            for other in all_allergens:
                if other != allergen:
                    all_allergens[other] = all_allergens[other].difference(all_allergens[allergen])
