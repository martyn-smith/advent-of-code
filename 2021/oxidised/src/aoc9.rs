...
fn expand(pt: (usize, usize), visited: &mut HashSet<(usize, usize)>, input: &Array2<usize>) -> Vec<(usize, usize)> {
    let mut pts = vec![];
...
fn find_low_points(input: &Array2<usize>) -> Vec<(usize, usize)> {
    ...
    if input[[i_max,0]] < input[[i_max - 1,0]]
        && input[[i_max,0]] < input[[i_max,1]] {
            pts.push((i_max,0));
        }
    if input[[0,j_max]] < input[[1,j_max]]
        && input[[0,j_max]] < input[[0,j_max - 1]] {
            pts.push((0, j_max));
        }
...
pub fn part_2(input: &Array2<usize>) -> usize {
    ...
    let mut basins = pts.iter().map(|pt| expand(*pt, &mut visited, input)).collect::<Vec<Vec<(usize, usize)>>>();
